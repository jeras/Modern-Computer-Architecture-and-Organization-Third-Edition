// Ex__2_double_format.cpp: Answer to Ch 9 Ex 2.
...
void print_float(float f)
{
	...
	double mantissa_dec;
	if (exponent == 0) // This is zero or a subnormal
	{
...
int main(void)
{
	...
	print_float(0);
	print_float(-0); // Minus sign is ignored, though IEEE 754 supports signed zero
	print_float(1);
	print_float(-1);
	print_float(6.674e-11f);
	print_float(1.0e38f);
	//print_float(1.0e39f); // Compile-time error: floating constant out of range
	print_float(1.0e-38f);
	print_float(1.0e-39f);

	print_double(0);
	print_double(-0); // Minus sign is ignored, though IEEE 754 supports signed zero
	print_double(1);
	print_double(-1);
	...
	//print_double(1.0e309); // Compile-time error: floating constant out of range
