...
Modify the program from [Exercise 1](Ex__1_float_format.md) to also accept a double-precision floating-point number and print the sign, exponent (with the bias removed), and mantissa. Test with the same input values as in [Exercise 1](Ex__1_float_format.md), and also with the values 1.0e308, 1.0e309, 1.0e-308, and 1.0e-309.

# Answer
See the C++ file [Ex__2_double_format.cpp](Code%20Files/Ex__2_double_format.cpp).
...
* The value `1.0e309` is not shown because using it causes a compile-time error: _The floating constant is out of range_.
...
* Numerically, subnormal doubles use a biased exponent of 1, which translates to an unbiased exponent of -1022.