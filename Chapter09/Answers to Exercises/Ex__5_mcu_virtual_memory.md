__Modern Computer Architecture and Organization Third Edition__, by Jim Ledin. Published by Packt Publishing.
# Chapter 9, Exercise 5

Do the i.MX RT1060 processors support the concept of paged virtual memory? Explain your answer.

# Answer
The i.MX RT1060 processors utilize physical memory addressing, supporting up to 16 memory protection regions. **These processors do not support paged virtual memory.**